...
import es.upm.oeg.epnoi.matching.metrics.domain.{Metadata, AuthorProfile, TopicDistribution}
...
class AuthorsSimilarityTest extends FunSuite {
  ...
  test("Same distributions and different number of authors") {
    ...
    val p4 = AuthorProfile(Authors.a4,Map((metadata,TopicDistribution(Array(0.4, 0.2)))))
